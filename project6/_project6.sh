...
do
    ...
    if do-compile --test-category "Warning" --test-message "Successful compile with $TEST" make
    then

        # For FAT read test...
        unzip -o $TEST_DIR/fat.img.zip


        echo Executing tests...
        # Run in background because qemu doesn't work well run in foreground with a timeout in effect
        qemu-system-i386 -nographic -drive file=xv6.img,index=0,media=disk,format=raw -drive file=fs.img,index=1,media=disk,format=raw -drive file=fat.img,index=2,media=disk,format=raw -smp 1 -device isa-debug-exit,iobase=0xf4,iosize=0x04 &
        QPID=$!
        countdown=5
