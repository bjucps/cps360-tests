...
void mrdTests() 
{
  startSection("RAM Disk: Multiple RAMDisk Tests");

  int fd = open("/f1", O_CREATE | O_WRONLY);
  write(fd, "abcde", 5);
  close(fd);


  // Create first extra ramdisk
  doexec("mrd rd1");
  doexec("cp /f1 /rd1/f1");
  check(compare_files("/f1", "/rd1/f1"), "First RAMDisk Ok");  

  // Create a second ramdisk
  doexec("mrd rd2");
  doexec("cp /f1 /rd2/f2");
  check(compare_files("/f1", "/rd2/f2"), "Second RAMDisk Ok");  
